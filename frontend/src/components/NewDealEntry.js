...
const NewDealEntry = ({ setDeals }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    brokerageFee: '',
    brokerageFeePaidTo: '',
    // New broker fee tracking fields
    brokerFee: {
      amount: '',
      brokerId: '',
      brokerName: '',
  ...
  const [sendSellerUpload, setSendSellerUpload] = useState(false);
  ...
  // Helper function to check if seller upload should be shown
  const shouldShowSellerUpload = () => {
    return (
      (formData.dealType === 'wholesale-flip' && formData.sellerType === 'private') ||
      (formData.dealType === 'retail-pp' && formData.dealType2SubType === 'buy')
    );
  };
  ...
  // Function to get appropriate dealType2SubOptions based on deal type
  const getDealType2SubOptions = (dealType) => {
    switch (dealType) {
      case 'wholesale-d2d':
        return [
          { value: 'buy', label: 'Buy' },
  ...
  const dealType2SubOptions = getDealType2SubOptions(formData.dealType);
  ...
  // Add current user to salesperson options if not already present
  const currentUserDisplayName = getUserDisplayName(currentUser);
  if (currentUserDisplayName && !salespersonOptions.find(option => option.value === currentUserDisplayName)) {
    salespersonOptions.push({ value: currentUserDisplayName, label: currentUserDisplayName });
  }

    // Set default salesperson to current user if available
    useEffect(() => {
      if (currentUser && !formData.salesperson) {
        const userDisplayName = getUserDisplayName(currentUser);
        
        if (userDisplayName) {
          setFormData(prev => ({
            ...prev,
            salesperson: userDisplayName
          }));
        }
      }
    }, [currentUser, formData.salesperson]);
  ...
  const handleInputChange = (field, value) => {
    ...
    if (['purchasePrice', 'listPrice', 'instantOffer', 'brokerageFee', 'payoffBalance', 'amountDueToCustomer', 'amountDueToRP'].includes(field)) {
      processedValue = formatCurrency(value);
    }
    // Handle wholesale price formatting (exclude for wholesale D2D buy deals)
    if (field === 'wholesalePrice' && !(formData.dealType === 'wholesale-d2d' && formData.dealType2SubType === 'buy')) {
      processedValue = formatCurrency(value);
    }
    ...
    if (field === 'dealType') {
      ...
      setFormData(prev => {
        ...
        // Determine funding source based on deal type
        let fundingSource = prev.fundingSource; // Keep existing if already set
        if (processedValue.startsWith('wholesale')) {
          fundingSource = 'flpn-wholesale';
          console.log('[DEAL TYPE CHANGE] 🔄 Auto-setting funding source to FLPN-Wholesale for wholesale deal');
        } else if (processedValue.startsWith('retail')) {
          fundingSource = 'flpn-retail';
          console.log('[DEAL TYPE CHANGE] 🔄 Auto-setting funding source to FLPN-Retail for retail deal');
        } else if (processedValue === 'auction') {
          fundingSource = 'cash';
          console.log('[DEAL TYPE CHANGE] 🔄 Auto-setting funding source to Cash for auction deal');
        }
        ...
        const newFormData = {
          ...
          // Auto-set funding source based on deal type
    ...
    setFormData(prev => {
      ...
      let newFormData = { ...prev, [field]: processedValue };
      
      // Update brokerFee structure when brokerageFee amount changes
      if (field === 'brokerageFee') {
        newFormData = {
          ...newFormData,
          brokerFee: {
            ...newFormData.brokerFee,
  ...
  const handleSaveWithData = async (dataToUse, isDraft = false) => {
    ...
    if (!isDraft && !validateForm(dataToUse)) {
      ...
      analytics.trackError('deal_validation_failed', Object.keys(formErrors).join(', '), { dealType: dataToUse.dealType });
      return;
    }
    ...
    try {
      ...
      const dealData = {
        ...
        instantOffer: parseFloat(dataToUse.instantOffer) || 0,
        wholesalePrice: (dataToUse.dealType === 'wholesale-d2d' && dataToUse.dealType2SubType === 'buy') ? 0 : parseFloat(dataToUse.wholesalePrice) || 0,
        ...
        salesperson: dataToUse.salesperson || getUserDisplayName(currentUser) || 'Unknown',
        // --- Add all financial fields below ---
        financial: {
          purchasePrice: parseFloat(dataToUse.purchasePrice) || 0,
          listPrice: parseFloat(dataToUse.listPrice) || 0,
          expectedMargin: 0, // Calculate if needed
          commission: {
            rate: parseFloat(dataToUse.commissionRate) || 0,
            estimatedAmount: 0 // Calculate if needed
          },
          // Broker Fee Tracking
          brokerFee: {
            amount: parseFloat(dataToUse.brokerageFee) || 0,
            brokerId: dataToUse.brokerFee?.brokerId || null,
            brokerName: dataToUse.brokerFee?.brokerName || dataToUse.brokerageFeePaidTo || '',
            brokerEmail: dataToUse.brokerFee?.brokerEmail || '',
            paid: false,
            paidDate: null,
            notes: ''
          }
      ...
      console.log('[HANDLE SAVE] API URL:', `${API_BASE}/api/sales/deals`);
      ...
      // Create deal using the sales tracker endpoint for broker fee tracking
      const dealResponse = await fetch(`${API_BASE}/api/sales/deals`, {
        method: 'POST',
        headers: buildHeaders(),
      ...
      // Track successful deal creation
      analytics.trackDealEvent('deal_created', {
        dealType: dataToUse.dealType,
        year: dataToUse.year,
        make: dataToUse.make,
        model: dataToUse.model,
      ...
      if (sendSellerUpload && formData.sellerEmail?.trim()) {
        if (formData.dealType === 'retail-d2d' && formData.dealType2SubType === 'buy') {
          ...
          const uploadResponse = await fetch(`${API_BASE}/api/seller-upload/generate-link`, {
            method: 'POST',
            headers: buildHeaders(),
            body: JSON.stringify({
              dealId: dealResult.deal._id,
          ...
          if (uploadData.success) {
            console.log('[DEAL SUBMIT] Seller upload link sent successfully');
            toast.success('Seller document upload link sent successfully!');
          } else {
            console.error('[DEAL SUBMIT] Failed to send seller upload link:', uploadData.message);
            toast.error('Deal submitted but seller upload link failed to send. Please contact support.');
          }
        } catch (uploadError) {
          console.error('[DEAL SUBMIT] Error sending seller upload link:', uploadError);
          toast.error('Deal submitted but seller upload link failed to send. Please contact support.');
        }
      }
      }
      ...
      <div className="space-y-2">
        ...
        {type === 'select' ? (
          ...
          <div className="relative">
            {(['purchasePrice', 'listPrice', 'instantOffer', 'brokerageFee', 'payoffBalance', 'amountDueToCustomer', 'amountDueToRP'].includes(field) || 
              (field === 'wholesalePrice' && !(formData.dealType === 'wholesale-d2d' && formData.dealType2SubType === 'buy'))) && (
            ...
            <input
              ...
              className={`w-full bg-white/10 border rounded-lg px-3 py-2 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors ${
                formErrors[field] ? 'border-red-500' : 'border-white/20 hover:border-white/30'
              } ${field === 'vin' ? 'pr-20' : ''} ${(['purchasePrice', 'listPrice', 'instantOffer', 'brokerageFee', 'payoffBalance', 'amountDueToCustomer', 'amountDueToRP'].includes(field) || 
                (field === 'wholesalePrice' && !(formData.dealType === 'wholesale-d2d' && formData.dealType2SubType === 'buy'))) ? 'pl-8' : ''}`}
            />
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="bg-white/5 rounded-xl p-6 border border-white/10">
              <div className="mb-4">
                <h3 className="text-lg font-medium text-white">Deal Type Selection</h3>
              </div>
                  ...
                  <div className={`mt-2 p-3 border rounded-lg ${(!formData.dealType2SubType || formData.dealType2SubType.trim() === '') ? 'bg-yellow-500/10 border-yellow-500/30' : 'bg-blue-500/10 border-blue-500/20'}`}>
                    <p className={`text-sm ${(!formData.dealType2SubType || formData.dealType2SubType.trim() === '') ? 'text-yellow-300' : 'text-blue-300'}`}>
                      {(!formData.dealType2SubType || formData.dealType2SubType.trim() === '') ? (
                        <>
                          ⚠️ <strong>Required:</strong> Please select "Buy" or "Sale" for {formData.dealType} deals:
                          <br />• Select <strong>"Buy"</strong> if RP Exotics is purchasing from {formData.dealType === 'wholesale-d2d' ? 'another dealer' : formData.dealType === 'retail-d2d' ? 'another dealer (to list for retail)' : 'another party'}
                          {formData.dealType !== 'retail-d2d' && (
                            <>
                              <br />• Select <strong>"Sale"</strong> if RP Exotics is selling to another party
                            </>
                          )}
                        </>
                      ) : formData.dealType2SubType === 'buy' ? (
                        <>
                          ✅ <strong>Buy Deal:</strong> RP Exotics is purchasing from {formData.dealType === 'wholesale-d2d' ? 'another dealer' : formData.dealType === 'retail-d2d' ? 'another dealer (to list for retail)' : 'another party'}
                          <br />• <strong>Seller:</strong> {formData.dealType === 'wholesale-d2d' ? 'Another dealer' : formData.dealType === 'retail-d2d' ? 'Another dealer' : 'Another party'} (you'll enter their info)
                          <br />• <strong>Buyer:</strong> RP Exotics (no buyer info needed)
                        </>
                      ) : formData.dealType2SubType === 'sale' ? (
                        <>
                          ✅ <strong>Sale Deal:</strong> RP Exotics is selling to another party
                          <br />• <strong>Seller:</strong> RP Exotics (no seller info needed)
                          <br />• <strong>Buyer:</strong> Another party (you'll enter their info)
                        </>
                      ) : formData.dealType === 'wholesale-flip' ? (
                        <>
                          💡 <strong>Important:</strong> This is how a wholesale flip deal works:
                          <br />• <strong>Buying</strong> from either a private party or dealer, then
                          <br />• <strong>Selling</strong> to a dealer for a quick flip
                        </>
                      ) : (
                        <>
                          💡 <strong>Important:</strong> Please select the correct option for {formData.dealType} deals:
                          <br />• Select <strong>"Buy"</strong> if RP Exotics is purchasing from {formData.dealType === 'wholesale-d2d' ? 'another dealer' : formData.dealType === 'retail-d2d' ? 'another dealer (to list for retail)' : 'another party'}
                          {formData.dealType !== 'retail-d2d' && (
                            <>
                              <br />• Select <strong>"Sale"</strong> if RP Exotics is selling to another party
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="relative">
                  ...
                  {formData.fundingSource && (
                    (formData.dealType?.startsWith('wholesale') && formData.fundingSource === 'flpn-wholesale') ||
                    (formData.dealType?.startsWith('retail') && formData.fundingSource === 'flpn-retail') ||
                    (formData.dealType === 'auction' && formData.fundingSource === 'cash')
                  ) && (
          ...
          <div className="bg-white/5 rounded-xl p-6 border border-white/10">
            ...
            {formData.dealType2SubType === 'sale' && formData.dealType !== 'wholesale-d2d' ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-6">
                  {renderFormField(dynamicLabels.purchasePriceLabel, 'purchasePrice', 'number', null, true, DollarSign)}
                  {renderFormField('Instant Offer', 'instantOffer', 'number')}
              ...
              // Financial info for retail-d2d deals - simplified layout without payoff and amount fields
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Left column */}
                <div className="space-y-6">
                  {renderFormField(dynamicLabels.purchasePriceLabel, 'purchasePrice', 'number', null, true, DollarSign)}
              ...
              // Financial info for wholesale D2D buy deals - exclude List Price and payoff/amount fields
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Left column */}
                <div className="space-y-6">
                  {renderFormField(dynamicLabels.purchasePriceLabel, 'purchasePrice', 'number', null, true, DollarSign)}
                  {renderFormField('Instant Offer', 'instantOffer', 'number')}
                  {/* Wholesale Price removed for wholesale D2D buy deals */}
              ...
              // Financial info for wholesale D2D sale deals - single column layout with 3 fields stacked
              <div className="space-y-6">
                {renderFormField(dynamicLabels.purchasePriceLabel, 'purchasePrice', 'number', null, true, DollarSign)}
                {renderFormField('Brokerage Fee', 'brokerageFee', 'number')}
                {renderFormField('Brokerage Fee Paid To', 'brokerageFeePaidTo', 'text')}
            ...
            <div className="bg-white/5 rounded-xl p-6 border border-white/10 space-y-10">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {formData.buyerType === 'dealer' && !isWholesaleBuyDeal() && !(formData.dealType === 'retail-d2d' && formData.dealType2SubType === 'buy') && renderFormField('Dealer Tier', 'buyerTier', 'select', [
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {formData.sellerType === 'dealer' && !(formData.dealType === 'wholesale-flip' && formData.dealType2SubType === 'buy-sell') && !isWholesaleBuyDeal() && !(formData.dealType === 'retail-d2d' && formData.dealType2SubType === 'buy') && renderFormField('Dealer Tier', 'sellerTier', 'select', [
                    { value: 'Tier 1', label: 'Tier 1: Pay Upon Title' },
                    { value: 'Tier 2', label: 'Tier 2: Pay Prior to Release' }
                  ])}
                </div>

                {/* Seller Document Upload Button */}
                {shouldShowSellerUpload() && (
                  <div className="mt-6 p-4 bg-blue-900/20 border border-blue-700/30 rounded-lg">
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="text-sm font-medium text-blue-300">📄 Seller Document Upload</h4>
                    </div>
                    ...
                    <div className="space-y-3">
                      <label className="flex items-center space-x-2 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={sendSellerUpload}
                          onChange={(e) => setSendSellerUpload(e.target.checked)}
                          className="rounded bg-white/10 border-white/20 text-blue-500 focus:ring-blue-500"
                        />
                        <span className="text-gray-300 text-sm">Send document upload link to seller when deal is submitted</span>
                      </label>

                      <p className="text-gray-400 text-xs">
                        Check the box above to automatically send a secure upload link to the seller when the deal is submitted.
                      </p>
                    </div>
                  </div>
                )}
            ...
            <div className="bg-white/5 rounded-xl p-6 border border-white/10">
              ...
              {isRetailPP ? (
                <>
                  ...
                  {/* Seller Document Upload for Retail PP */}
                  {shouldShowSellerUpload() && (
                    <div className="mt-6 p-4 bg-blue-900/20 border border-blue-700/30 rounded-lg">
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="text-sm font-medium text-blue-300">📄 Seller Document Upload</h4>
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {(formData.dealType === 'retail-d2d' || formData.dealType === 'wholesale-d2d') && (
                      <>
                        ...
                        {/* Dealer Tier for retail-d2d deals - only for sale deals, not buy deals */}
                        {formData.dealType === 'retail-d2d' && formData.dealType2SubType === 'sale' && (
                          <div>
                  ...
                  {/* Seller Document Upload for Other Deal Types */}
                  {shouldShowSellerUpload() && (
                    <div className="mt-6 p-4 bg-blue-900/20 border border-blue-700/30 rounded-lg">
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="text-sm font-medium text-blue-300">📄 Seller Document Upload</h4>
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black">
      ...
      <div className="relative max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="bg-white/5 backdrop-blur-lg rounded-2xl border border-white/10 p-8">
          ...
          sellerEmail: 'info@rpexotics.com',
